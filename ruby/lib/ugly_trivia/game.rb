module UglyTrivia
  class Player
    attr_accessor :name, :in_penalty_box, :place
    def initialize(name)
      @name = name
  ...
  class Game
    ...
    def is_getting_out_of_penalty_box_text(roll)
      is_getting_out_of_penalty_box(roll) ? '' : ' not'
    end

    def is_getting_out_of_penalty_box(roll)
      roll % 2 != 0
    end
    ...
    def roll(roll)
      ...
      if @players[@current_player].in_penalty_box
        @is_getting_out_of_penalty_box = is_getting_out_of_penalty_box(roll)
        puts "#{@players[@current_player]} is#{is_getting_out_of_penalty_box_text(roll)} getting out of the penalty box"
      end

      unless @players[@current_player].in_penalty_box && roll % 2 == 0
        @places[@current_player] = @places[@current_player] + roll
        @players[@current_player].place = @places[@current_player] + roll
        @places[@current_player] = @places[@current_player] - 12 if @places[@current_player] > 11
        @players[@current_player].place = @places[@current_player] - 12 if @places[@current_player] > 11
    ...
    def wrong_answer
      ...
      @players[@current_player].in_penalty_box = true
